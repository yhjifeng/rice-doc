...
1. 插件如果在升级后或者更换核心,或者更换核心小版本后请务必执行/plk up 进行数据转换(1.11.0+版本新增这个命令)
2. 插件不要使用yum这类插件进行热加载！！！ 
3.任务目标, 有些物品手上拿着和放下去是不同物品,所以检测不到,建议这里的话尽量不要使用这些特殊物品作为任务目标,例如:红石粉,铁轨,种子...
